...
client.on('message', msg => {
  if (msg.content.toLowerCase() === 'sa') {
		if (!msg.guild.member(msg.author).hasPermission("SEND_MESSAGES")) {
			msg.author.sendMessage('**Aleyküm Selam,** Hoş Geldin  :tada: :blush:    '); 
		} else {
		msg.channel.send('**Aleyküm Selam,** Hoş Geldin  :tada: :blush:    ');
		}
	}
});
...
client.on("guildMemberAdd", member => {
	...
	member.send("**Aramıza hoş geldin! İyi Vakit Geçirmen Dileğiyle^^**")
...
client.on('guildCreate', guild => {
  ...
  .addField('**Lhirox Bot sunucunuza eklendi!**', `${owner}`)
  .addField('**Lhirox Bot** sunucunuzdaki insanlara kolaylıklar sağlar.', `**${prefix}bilgi** yazmanız yeterlidir!`)
